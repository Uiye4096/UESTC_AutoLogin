...
var _netPass = new WeakMap();

var _checkAgreementStatus = new WeakMap();
...
var _loginHuawei = new WeakMap();

var _loginEvent = new WeakMap();

var _loginSelfServiceQR = new WeakMap();

var _sendVcodePhone = new WeakMap();

var _sendVcodeAccount = new WeakMap();

var _sendVcodeEvent = new WeakMap();
...
var Portal = /*#__PURE__*/function (_Utils) {
  ...
  function Portal(config) {
    ...
    _defineProperty(_assertThisInitialized(_this), "doubPull", function () {
      _this.ajax.post({
        url: _classPrivateFieldGet(_assertThisInitialized(_this), _api).doubPull,
        params: {
          user_name: _this.userInfo.username,
          ipv4: _this.userInfo.ip,
          ipv6: _this.userInfo.otherStackIP,
          ac_id: _this.portalInfo.acid
        },
        success: function success(res) {
          // 若当前页为认证页，跳转至成功页
          if (CONFIG.page === 'account') _this.toSuccess();
        },
        error: function error(res) {// this.confirm(this.translate(res))
        }
      });
    });
    ...
    _api.set(_assertThisInitialized(_this), {
      ...
      value: {
        ...
        // GET   | 事件访客认证
        authEvent: '/cgi-bin/srun_events_auth',
        // jsonp | 获取 Token
        ...
        // GET   | 事件访客发送短信
        vcodeEvent: '/cgi-bin/srun_mobile_events_code',
        // GET   | 获取 Sign
        ...
        // POST  | 华为认证
        huaweiAuth: '/v1/huaweiNce',
        // GET   | 代拨日志
        diallog: '/v1/srun_portal_diallog',
        // GET   | 页面黑白设置
        bgSwitch: '/v2/srun_portal_bg_switch',
        // GET   | 判断是否需要验证码
        checkCaptcha: '/v2/srun_portal_captcha_image_info',
        // GET   | 获取图片验证码
        captcha: '/v2/srun_portal_captcha_image',
        // POST  | 双栈互拉上线
        doubPull: '/v1/double-stack-online'
      }
    });
          ...
          error: function error() {
            if (_this.portalInfo.doub) {
              ...
              _this.ajax.jsonp({
                ...
                url: _classPrivateFieldGet(_assertThisInitialized(_this), _api).info,
                success: function success(res) {
                  if (CONFIG.isIPV6) that.userInfo.otherStackIP = res.online_ip || '';
                  if (!CONFIG.isIPV6) that.userInfo.otherStackIP = res.online_ip6 || '';

                  _classPrivateFieldGet(that, _doubSSO).call(that, true);
                },
                error: function error(res) {
                  that.userInfo.otherStackIP = res.online_ip || '';
            ...
            if (!_this.portalInfo.doub) _classPrivateFieldGet(_assertThisInitialized(_this), _doubSSO).call(_assertThisInitialized(_this), false);
          }
        });
      }
    });

    _doubSSO.set(_assertThisInitialized(_this), {
      writable: true,
      value: function value(isDoub) {
        var ip = '';

        if (isDoub) {
          if (CONFIG.isIPV6) {
            ip = _this.userInfo.otherStackIP && _this.userInfo.ip ? "&double_ip=".concat(_this.userInfo.otherStackIP, ",").concat(_this.userInfo.ip) : "&double_ip=".concat(_this.userInfo.otherStackIP || _this.userInfo.ip || '');
          } else {
            ip = _this.userInfo.ip && _this.userInfo.otherStackIP ? "&double_ip=".concat(_this.userInfo.ip, ",").concat(_this.userInfo.otherStackIP) : "&double_ip=".concat(_this.userInfo.ip || _this.userInfo.otherStackIP || '');
          }
        }

        _this.ajax.get({
          url: _classPrivateFieldGet(_assertThisInitialized(_this), _api).sso + location.search + ip,
          success: function success(res) {
            _this.setCookie('isCasLogin', true);

            if (res.Redirect) location.href = res.Redirect;

            var sendCheckOnlineInfo = function sendCheckOnlineInfo() {
              _this.ajax.jsonp({
                url: _classPrivateFieldGet(_assertThisInitialized(_this), _api).info,
                success: function success() {
                  location.href = "./srun_portal_success?ac_id=".concat(res.ID);
                }
              });
            };
            ...
            _this.checkOnlineTimer = setInterval(function () {
              ...
              if (checkNum === 0) {
                clearInterval(that.checkOnlineTimer);

                _this.confirm({
                  message: _this.translate('未查询到在线信息，请刷新页面重试'),
    ...
    _selectDefaultLang.set(_assertThisInitialized(_this), {
      ...
      value: function value() {
        ...
        if (!_this.getCookie('lang') && typeof lang === 'string' && lang.includes('en')) _this.portalInfo.lang = 'en-US';
        if (!_this.getCookie('lang') && typeof lang === 'string' && lang.includes('en')) _this.portalInfo.lang = 'zh-CN'; // $('#change-lang').val(this.portalInfo.lang);
    ...
    _logoutDm.set(_assertThisInitialized(_this), {
      ...
      value: function value(obj) {
        var _this$portalInfo = _this.portalInfo,
            ...
            ipv6 = _this$portalInfo.ipv6,
            doub = _this$portalInfo.doub,
            nowType = _this$portalInfo.nowType; // 检查次数

        var checkNum = 3; // 已完成请求

        var finishReq = 0; // 认证成功的消息

        var successMsg = '';
        $('.btn-logout').html("<div class=\"loader\">".concat(_this.translate('LogoutLoading'), "...</div>")); // 注销完成

        var logoutClose = function logoutClose() {
          _this.loading.logout = false;
          clearInterval(_this.checkOnlineTimer);
          $('.btn-logout').html(_this.translate('Logout'));
        }; // 请求是否均得到响应


        var noPending = function noPending() {
          return !doub && finishReq === 1 || doub && finishReq === 2;
        }; // 发送下线请求


        var sendLogout = function sendLogout(host) {
          var time = Date.parse(new Date()) / 1000;
          var unbind = 1; // 优先使用指定 IP

          var ip = obj.ip || _this.getUserIP(host);
          ...
          try {
            _this.ajax.jsonp({
              ...
              error: function error(res) {
                ...
                _this.loading.logout = false; // 若所有请求均已完成

                if (noPending()) logoutClose(); // 注销失败给出弹框
            ...
            if (noPending()) logoutClose();
          }
        }; // 使用 Portal 页面 IP 类型注销


        sendLogout(nowType === 'ipv4' ? ipv4 : ipv6); // 若符合双栈注销，则进行双栈注销，发起另一类型注销

        if (doub && _this.userInfo.otherStackIP) sendLogout(nowType === 'ipv4' ? ipv6 : ipv4);

        var sendCheckOnline = function sendCheckOnline() {
          // 查询在线表
          _this.ajax.jsonp({
            url: _classPrivateFieldGet(_assertThisInitialized(_this), _api).info,
            params: {
            ...
            error: function error(res) {
              logoutClose(); // 注销成功，传入回调执行回调

              if (obj.success) obj.success(successMsg); // 注销成功，未传入回调，自动返回认证页面
        ...
        _this.checkOnlineTimer = setInterval(function () {
          console.info('准备查询在线信息', checkNum);
          checkNum -= 1; // 若请求均已得到响应

          if (noPending()) {
            console.info('请求均已得到响应，发起查询');
            sendCheckOnline();
          } // 轮训结束
    ...
    _logoutNormal.set(_assertThisInitialized(_this), {
      ...
      value: function value(obj) {
        var _this$portalInfo2 = _this.portalInfo,
            ...
            ipv6 = _this$portalInfo2.ipv6,
            doub = _this$portalInfo2.doub,
            nowType = _this$portalInfo2.nowType; // 检查次数
          ...
          navigator.userAgentData.getHighEntropyValues(['platformVersion']).then(function (ua) {
            if (navigator.userAgentData.platform === 'Windows') {
              ...
              if (majorPlatformVersion >= 13) {
                _this.portalInfo.userDevice.device = 'Windows 11';
              } else if (majorPlatformVersion > 0) {// Windows 10
              } else {// Before Windows 10
                }
            } // Not running on Windows

          });
        } catch (error) {// Not support
          // 1. https + 可信 ssl 证书 + 域名访问
          // 2. Chromium 内核浏览器
        }
    ...
    _getToken.set(_assertThisInitialized(_this), {
      writable: true,
      value: function value(host, callback) {
        _this.ajax.jsonp({
          host: host,
    ...
    _getSign.set(_assertThisInitialized(_this), {
      ...
      value: function value(params, callback) {
        var _error = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : function (err) {};
      ...
      value: function value(obj) {
        _this.ajax.get({
          ...
          success: function success(res) {
            ...
            _classPrivateFieldGet(_assertThisInitialized(_this), _polling).call(_assertThisInitialized(_this), 1, res.data.expire_time);
          },
          error: function error(res) {
            _this.confirm(_this.translate(res));
          }
    ...
    _netPass.set(_assertThisInitialized(_this), {
      writable: true,
      value: function value() {
        ...
        _this.ajax.get({
          ...
          success: function success(_ref) {
            ...
            _this.ajax.jsonp({
              ...
              success: function success() {
                _this.ajax.get({
                  url: '/v1/srun_weixin_official_url',
                  success: function success(res) {
                    location.href = res.official_url;
                  }
                });
              },
              error: function error(err) {
                _this.confirm(_this.translate(err));
              }
            });
          },
          error: function error(err) {
    ...
    _checkAgreementStatus.set(_assertThisInitialized(_this), {
      writable: true,
      value: function value(callback) {
        // 查询防抖
        if (_this.loading.checkProt) return; // 若未勾选协议

        if (!$('#protocol').prop('checked')) return _this.confirm(_this.translate('ReadAgreement')); // 更改查询状态为进行中

        _this.loading.checkProt = true; // 发起查询请求
        ...
        _this.ajax.get({
          ...
          success: function success(res) {
            // 更改查询状态为结束
            _this.loading.checkProt = false; // 用户同意了哪些协议
    ...
    _loginCisco.set(_assertThisInitialized(_this), {
      ...
      value: function value(obj) {
        _this.ajax.post({
          ...
          params: {
            user_name: _this.userInfo.username || _this.userInfo.phone,
            password: _this.getUserPassword() || _this.userInfo.vcode
          },
          success: function success() {
            _this.loading.login = false;
            $('#form-cisco').submit(); // 认证成功，传入回调则执行回调
    ...
    _loginAccount.set(_assertThisInitialized(_this), {
      ...
      value: function value(obj) {
        ...
        var password = _this.getUserPassword();

        var ac_id = _this.portalInfo.acid;

        var ap_id = _this.getUrlParams('ap_id');

        var ap_ip = _this.getUrlParams('ap_ip'); // 当前设备 IPv4 IPv6 地址


        var _this$portalInfo3 = _this.portalInfo,
            ipv4 = _this$portalInfo3.ipv4,
            ipv6 = _this$portalInfo3.ipv6,
            doub = _this$portalInfo3.doub,
            nowType = _this$portalInfo3.nowType; // 检查次数
        ...
        var successMsg = '';
        $('.btn-login').html("<div class=\"loader\">".concat(_this.translate('LoginLoading'), "...</div>")); // 认证完成

        var authClose = function authClose() {
          _this.loading.login = false;
          clearInterval(_this.checkOnlineTimer);
          $('.btn-login').html(_this.translate('Login'));
        }; // 请求是否均得到响应
        ...
        var sendAuth = function sendAuth(host) {
          // 双栈认证时 IP 参数为空
          var ip = _this.getUserIP(host); // 获取 Token


          _classPrivateFieldGet(_assertThisInitialized(_this), _getToken).call(_assertThisInitialized(_this), host, function (token) {
            // 用户密码 MD5 加密
            ...
            try {
              ...
              _this.ajax.jsonp({
                ...
                params: {
                  ...
                  n: n,
                  type: type,
                  captchaId: _this.portalInfo.captchaId,
                  captchaVal: _this.userInfo.captchaVal,
                  ap_id: ap_id,
                  ap_ip: ap_ip,
                  mac: _this.userInfo.mac
                },
                success: function success(res) {
                  ...
                  }); // 如果参数中存在 location，则重定向

                  if (res.location) location.href = res.location; // 翻译后的认证成功信息
                ...
                error: function error(res) {
                  ...
                  if (res.ecode === 'E2620' && CREATER.useOnlineDeviceMgr) return _this.confirm({
                    message: _this.translate('ExceededOnlineNumber'),
                    confirm: function confirm() {
                 ...
                 // if (res.error_msg === 'E2901: (Third party -101)User name or password is empty.') {
                 //   _this.bindCarrier();

                   // _this.dialog.open('bindCarrier');
                  //} // IP 已经在线了 - 重新认证
                  ...
                  if (res.error_msg === 'user_must_modify_password' || res.error === 'user_must_modify_password') return _this.confirm({
                    ...
                    confirm: function confirm() {
                      $('.change-password #dialog-username').val(_this.userInfo.username);
                      $('.change-password #dialog-vcode-pass').val(_this.getUserPassword());
                      $('#forget').click();
                    },
                    cancel: function cancel() {}
                  }); // 若验证码错误

                  if (res.error === 'vcode_error') _this.getCaptcha(); // 若账号锁定

                  if (res.error === 'account_locked') return _this.confirm(_this.translate('AccountLocked') + res.error_msg + _this.translate('Retry')); // 错误提示
        ...
        sendAuth(nowType === 'ipv4' ? ipv4 : ipv6); // 若符合双栈认证，则进行双栈认证，发起另一类型认证

        if (doub && _this.userInfo.otherStackIP) sendAuth(nowType === 'ipv4' ? ipv6 : ipv4); // 发送查在线请求
        ...
        var sendCheckOnline = function sendCheckOnline() {
          ...
          _this.ajax.jsonp({
            ...
            success: function success(res) {
              authClose(); // 认证成功，传入回调则执行回调

              if (obj.success) obj.success(successMsg); // 认证成功，未传入回调则自动重定向至 success
    ...
    _loginAccountSMS.set(_assertThisInitialized(_this), {
      ...
      value: function value(obj) {
        ...
        _this.ajax.get({
          ...
          success: function success(res) {
            // 翻译后的认证成功信息
            successMsg = _this.translate(res); // 轮训查询在线
    ...
    _loginPhoneSMS.set(_assertThisInitialized(_this), {
      ...
      value: function value(obj) {
        ...
        _classPrivateFieldGet(_assertThisInitialized(_this), _getSign).call(_assertThisInitialized(_this), params, function (token, sign) {
          ...
          params.nas_ip = _this.portalInfo.nasIp;
    ...
    _loginHuawei.set(_assertThisInitialized(_this), {
      writable: true,
      value: function value(obj) {
        // 检查次数
        var checkNum = 3; // 已完成请求
        ...
        var checkOnline = function checkOnline() {
          $('.btn-login').html("<div class=\"loader\">".concat(_this.translate('LoginLoading'), "...</div>")); // 轮训查询在线

          _this.checkOnlineTimer = setInterval(function () {
            console.info('准备查询在线信息', checkNum);
            checkNum -= 1;
            sendCheckOnline(); // 轮训结束

            if (checkNum === 0) authClose();
          }, 1000);
        };

        var sendToHuawei = function sendToHuawei() {
          _this.ajax.post({
            url: _classPrivateFieldGet(_assertThisInitialized(_this), _api).huaweiAuth,
            params: {
              loginUrl: decodeURIComponent(_this.getUrlParams('loginurl')),
              apMac: _this.getUrlParams('ap-mac'),
              ssid: _this.getUrlParams('ssid'),
              userIp: _this.getUrlParams('user-ip'),
              userMac: _this.getUrlParams('user-mac'),
              apLocation: _this.getUrlParams('ap-location'),
              user_name: _this.userInfo.username + _this.userInfo.domain
            },
            success: function success(res) {
              // 更改登录状态为结束
              checkOnline();
    ...
    _loginEvent.set(_assertThisInitialized(_this), {
      writable: true,
      value: function value(obj) {
        var params = {
          phone: _this.userInfo.username || _this.userInfo.phone,
          ...
          type: 'auth',
          t: Date.parse(new Date()) / 1000,
          event_id: _this.getUrlParams('random')
        }; // 认证成功的消息
        ...
        _classPrivateFieldGet(_assertThisInitialized(_this), _getSign).call(_assertThisInitialized(_this), params, function (token, sign) {
          ...
          _this.ajax.jsonp({
            url: _classPrivateFieldGet(_assertThisInitialized(_this), _api).authEvent,
            params: params,
            success: function success(res) {
              // 翻译后的认证成功信息
              successMsg = _this.translate(res); // 轮训查询在线
    ...
    _loginSelfServiceQR.set(_assertThisInitialized(_this), {
      writable: true,
      value: function value() {
        _this.portalInfo.acid = _this.portalInfo.acid || CONFIG.acid;
        _this.userInfo.username = _this.userInfo.username || CONFIG.username;
        _this.userInfo.domain = '';

        var key = _this.getUrlParams('key');

        var _this$userInfo = _this.userInfo,
            ip = _this$userInfo.ip,
            username = _this$userInfo.username;
        var _this$portalInfo4 = _this.portalInfo,
            ipv4 = _this$portalInfo4.ipv4,
            ipv6 = _this$portalInfo4.ipv6,
            nowType = _this$portalInfo4.nowType,
            acid = _this$portalInfo4.acid;
        var host = nowType === 'ipv4' ? ipv4 : ipv6;
        var checkNum = 3; // 获取 Token

        _classPrivateFieldGet(_assertThisInitialized(_this), _getToken).call(_assertThisInitialized(_this), host, function (token) {
          _this.ajax.get({
            url: '/v1/srun_portal_hmd5',
            params: {
              key: key,
              token: token,
              ip: ip
            ...
            success: function success(res) {
              ...
              _this.ajax.jsonp({
                ...
                error: function error(res) {
                  ...
                  if (res.error_msg === 'not_online_error') return _this.showLog();
                  if (res.error_msg === 'no_response_data_error') return _this.showLog();
                  if (res.error_msg === 'RD000') return _this.showLog(); // 错误提示
        ...
        _this.checkOnlineTimer = setInterval(function () {
          ...
          if (checkNum === 0) clearInterval(_this.checkOnlineTimer);
        }, 1000);
      }
    });
    ...
    _sendVcodeEvent.set(_assertThisInitialized(_this), {
      writable: true,
      value: function value(obj) {
        var params = {
          phone: obj.phone,
        ...
        _classPrivateFieldGet(_assertThisInitialized(_this), _getSign).call(_assertThisInitialized(_this), params, function (token, sign) {
          ...
          _this.ajax.jsonp({
            url: _classPrivateFieldGet(_assertThisInitialized(_this), _api).vcodeEvent,
            params: params,
            success: function success(res) {
              _this.loading.sendSMS = false;
              if (obj.success) obj.success(res);
    ...
    _setPortalInfo.set(_assertThisInitialized(_this), {
      ...
      value: function value() {
        // 设备当前 IP
        _this.userInfo.ip = _this.convertToIP(_this.config.ip) || _this.getCookie('ip') || _this.config.ip; // 用户 Mac 地址

        _this.userInfo.mac = _this.config.mac; // Nas IP

        _this.portalInfo.nasIp = _this.config.nas; // 语言类型

        _this.portalInfo.lang = _this.config.lang; // AC ID

        _this.portalInfo.acid = _this.config.acid; // theme

        _this.portalInfo.theme = _this.getUrlParams('theme'); // Nas IP

        _this.portalInfo.nasIp = _this.getUrlParams('nas_ip'); // 流量进位方式 1024 | 1000，默认 1024
    ...
    _showInfo.set(_assertThisInitialized(_this), {
      ...
      value: function value() {
        ...
        $('.panel-login .online-info').append(dom); // 同步通知高度
    ...
    _getAgentDialLog.set(_assertThisInitialized(_this), {
      ...
      value: function value() {
        var checkNum = 10;
        var checkDialLogTimer = setInterval(function () {
          checkNum -= 1;

          _this.ajax.get({
            url: _classPrivateFieldGet(_assertThisInitialized(_this), _api).diallog,
            params: {
              // 查询代拨日志不带域名
              username: _this.userInfo.username
            },
            success: function success(res) {
              clearInterval(checkDialLogTimer);
              $('.dial-log .dial-querying').hide();
              $('.dial-log .dial-success').css('display', 'inline-block');
            },
            error: function error(res) {
              if (res.code === 100 && checkNum === 0) {
                clearInterval(checkDialLogTimer);
                $('.dial-log .dial-querying').hide();
                $('.dial-log .dial-result').css('display', 'inline-block');
              }

              if (res.code !== 100) {
                clearInterval(checkDialLogTimer);
                $('.dial-log .dial-querying').hide();
                $('.dial-log .dial-fail').html(_this.translate(res) || '代拨失败').css('display', 'inline-block');
              }
            }
          });
        }, 500);
    ...
    _panelPay.set(_assertThisInitialized(_this), {
      ...
      value: function value() {
        ...
        $(document).on('click', '.portal-dialog.panel-prod .item-list li', function () {
          ...
          that.loading(that.translate('PackageChanging') + '......');
          that.ajax.post({
        ...
        $(document).on('click', '.portal-dialog.panel-pay .btn-confirm-pay', function () {
          ...
          if (Portal.mobile) {
            closePanelPay();
            closePanelProd();
            that.loading(that.translate('Paying'));
          }
        }); // 缴费按钮
        ...
        $(document).on('click', '#btn-pay', function () {
          ...
          _classPrivateFieldGet(that, _checkAgreementStatus).call(that, function () {
    ...
    _this.useOnlineDeviceMgr(); // 查询是否开启黑白页面模式


    _this.bgSwitch();

    if (_this.getUrlParams('theme') === 'debug') {
      Portal.debugMode = true;
      Portal.debugTheme = true;
    }

    if (Portal.debugMode) {
      _this.setLog('页面重新加载，当前 URL', location.href);

      if (Portal.debugTheme) {
        _this.confirm({
          title: 'Debug 模式',
          message: '请在问题复现后再导出日志!!! 若测试未完成，请点击继续 Debug，本弹窗刷新页面即可出现',
  ...
  _createClass(Portal, [{
    ...
    function bgSwitch() {
      ...
      $.ajax({
        url: _classPrivateFieldGet(this, _api).bgSwitch,
        type: 'get',
        data: '',
        headers: function () {
          var headers = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
          // 自定义请求头
          Object.keys(_this3.requestHead).forEach(function (key) {
            return headers[key] = _this3.requestHead[key];
          });
          return headers;
        }(),
        success: function success(res) {
          _this3.setLog("\u63A5\u53E3 /v2/srun_portal_bg_switch \u8BF7\u6C42\u6210\u529F\uFF0C\u54CD\u5E94\u53C2\u6570\u4E3A", res); // 使用 code 码做判断
          ...
          if (res.code !== undefined && res.error === undefined) {
            ...
            if (res.code === 0) {
              ...
              if (res.data === '1') {
                ...
                if (isIE) {
                  // IE变色
                  $('body').css('-ms-filter', 'gray');
                  $('body').css('filter', 'gray');
                  $('.section .panel-login button').css('filter', 'gray');
                  $('.header').css('position', 'static');
                  var scriptElement = $('<script></script>');
                  scriptElement.attr('src', './static/themes/pro/lib/grayscale.js');
        ...
        // 网络连接错误
        error: function error(res) {
          _this3.setLog("\u63A5\u53E3 /v2/srun_portal_bg_switch \u8BF7\u6C42\u5931\u8D25\uFF0C\u5931\u8D25\u539F\u56E0\u4E3A", res); // this.confirm(this.translate('NetErr'))
    ...
    key: "getUserPassword",
    value: function getUserPassword() {
      var base64 = this.clone($.base64);
      base64.setAlpha('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/');
      return CREATER.cryptography ? base64.encode(encodeURIComponent(this.userPassword)) : this.userPassword;
    }
    /**
     * 查询用户在线状态
     * @method
    ...
    value: function info() {
      ...
      if (this.config.page === 'selfServiceQR') return _classPrivateFieldGet(this, _loginSelfServiceQR).call(this);
      if (this.config.page === 'qrcode') _classPrivateFieldGet(this, _createAuthQr).call(this);
      if (this.config.page === 'wework') _classPrivateFieldGet(this, _createWeworkQr).call(this);
      if (this.config.page === 'official') _classPrivateFieldGet(this, _netPass).call(this); // 查询在线表

      this.ajax.jsonp({
        url: _classPrivateFieldGet(this, _api).info,
        params: Object.assign({
          ip: this.userInfo.ip
        }, obj.params),
        success: function success(res) {
          Portal.prototype.setLog("\u7528\u6237\u4E3A\u5728\u7EBF\u72B6\u6001"); // String 用户 Mac 地址

          _this4.userInfo.mac = res.user_mac; // String 用户账号 (不会携带域)

          _this4.userInfo.username = res.user_name; // String 真实姓名

          _this4.userInfo.realname = res.real_name; // String 用户域 Ex:域为账号 xingrong@cmcc 中的 @cmcc

          _this4.userInfo.domain = res.domain ? "@".concat(res.domain) : ''; // String 产品名称

          _this4.userInfo.productName = res.products_name; // String 产品 ID

          _this4.userInfo.productId = res.products_id; // String 计费策略名称

          _this4.userInfo.billingName = res.billing_name; // String 在线设备数量

          _this4.userInfo.deviceTotal = "".concat(res.online_device_total, "\u53F0"); // Number 余额

          _this4.userInfo.balance = res.user_balance.toFixed(2); // Number 结算日期

          _this4.userInfo.checkoutDate = res.checkout_date; // Number 已用流量 B

          _this4.userInfo.usedFlow = _this4.formatFlow(res.sum_bytes, _this4.portalInfo.flowMode); // Number已用时长 s

          _this4.userInfo.usedTime = _this4.formatTime(res.sum_seconds); // 用户另一栈的 IP (实际获取由使用另一栈发请求进行)

          _this4.userInfo.otherStackIP = ''; // 显示用户在线信息

          _classPrivateFieldGet(_this4, _showInfo).call(_this4); // 若传入在线回调函数，则执行回调函数


          if (obj.online) obj.online(res); // 在认证页面用户为在线状态则跳转到成功页面

          if (!_this4.online) return _this4.toSuccess();

          if (res.pppoe_dial !== '1' && CREATER.showDialLog) {
            $('.dial-log .dial-querying').hide();
            $('.dial-log .dial-fail').html('未绑定').css('display', 'inline-block');
          }

          if (res.pppoe_dial === '1' && CREATER.showDialLog) {
            return _classPrivateFieldGet(_this4, _getAgentDialLog).call(_this4);
          }
        },
        error: function error(res) {
          Portal.prototype.setLog("\u7528\u6237\u4E3A\u79BB\u7EBF\u72B6\u6001"); // 在成功页面用户为离线状态则跳转到认证页面

          if (_this4.online) return _this4.toIndex(); // 若传入离线回调函数，则执行回调函数

          if (obj.offline) obj.offline(res);
          if (res.error !== 'not_online_error') _this4.confirm(_this4.translate(res));
          _this4.userInfo.otherStackIP = res.online_ip;
        }
      });
    }
    /**
    ...
    key: "bindCarrier",
    value: function bindCarrier() {
      var _this5 = this;

      var dom = "\n        <div class=\"change-password\">\n            <div class=\"row\">\n                <label for=\"product-name\"></label>\n                <select class=\"input-text\" id=\"product-select\"></select>\n            </div>\n            <div class=\"row\">\n                <label for=\"carrier-account\"></label>\n                <input class=\"input-text\" type=\"text\" id=\"carrier-account\" placeholder=\"\u8FD0\u8425\u5546\u8D26\u53F7\"/>\n            </div>\n            <div class=\"row\">\n                <label for=\"re-carrier-account\"></label>\n                <input class=\"input-text\" type=\"text\" id=\"re-carrier-account\" placeholder=\"\u786E\u8BA4\u8FD0\u8425\u5546\u8D26\u53F7\"/>\n            </div>\n            <div class=\"row\">\n                <label for=\"carrier-password\"></label>\n                <input class=\"input-text\" type=\"password\" id=\"carrier-password\" placeholder=\"\u8FD0\u8425\u5546\u5BC6\u7801\"/>\n            </div>\n            <div class=\"row\">\n                <label for=\"re-carrier-password\"></label>\n                <input class=\"input-text\" type=\"password\" id=\"re-carrier-password\" placeholder=\"\u786E\u8BA4\u8FD0\u8425\u5546\u5BC6\u7801\"/>\n            </div>\n        </div>";
      ...
      var closeDialog = function closeDialog() {
        _this5.setLog('关闭了修改密码面板');

        _this5.dialog.close('bindCarrier');

        $('.component.promt[data-name=bindCarrier]').remove();
      }; // 获取用户名下已订购产品


      var getUserProducts = function getUserProducts() {
        _this5.ajax.get({
          url: '/v1/srun_portal_proxy',
          params: {
            uri: '/api/v1/package/users-packages',
            user_name: _this5.userInfo.username
          },
          success: function success(res) {
            var $select = $('#product-select'); // 清空现有选项（保留第一个提示选项）

            $select.find('option:not(:first)').remove(); // 添加产品选项
          ...
          error: function error(res) {
            _this5.setLog.err("\u83B7\u53D6\u4EA7\u54C1\u5931\u8D25");

            _this5.confirm(_this5.translate(res));
          }
        });
      };

      getUserProducts(); // 解绑运营商

      var unbindCarrier = function unbindCarrier(para) {
        _this5.ajax.get({
          url: '/v1/srun_portal_proxy',
          params: {
            uri: '/api/v1/product/operators',
            user_name: para.username,
            products_id: para.products_id,
            action: 0
          },
          success: function success(res) {
            _this5.setLog("\u8FD0\u8425\u5546\u89E3\u7ED1\u6210\u529F");

            closeDialog();

            _this5.confirm({
              title: '解绑成功',
              message: '是否要重新绑定运营商账号？',
              confirm: function confirm() {
                // 重新弹出绑定框
                _this5.bindCarrier();

                _this5.dialog.open('bindCarrier');
              },
              cancel: function cancel() {}
            });
          },
          error: function error(res) {
            _this5.setLog.err("\u8FD0\u8425\u5546\u89E3\u7ED1\u5931\u8D25");
      ...
      var toBindCarrier = function toBindCarrier(para) {
        _this5.ajax.get({
          url: '/v1/srun_portal_proxy',
          params: {
            uri: '/api/v1/product/operators',
            user_name: para.username,
            products_id: para.products_id,
            mobile_phone: para.mobile_phone,
            mobile_password: para.mobile_password
          },
          success: function success(res) {
            _this5.setLog("\u8FD0\u8425\u5546\u7ED1\u5B9A\u6210\u529F");
          ...
          error: function error(res) {
            _this5.setLog.err("\u8FD0\u8425\u5546\u7ED1\u5B9A\u5931\u8D25");

            if (res.code == 10202) {
              _this5.confirm({
                title: '提示',
                message: "".concat(_this5.translate(res), "; \u662F\u5426\u89E3\u7ED1\u5F53\u524D\u8FD0\u8425\u5546\u8D26\u53F7\uFF1F"),
                confirm: function confirm() {
                  return unbindCarrier(para);
                },
                cancel: function cancel() {}
              });
            } else {
      ...
      this.dialog.create({
        ...
        event: event,
        confirm: function confirm() {
          _this5.setLog("\u51C6\u5907\u63D0\u4EA4\u4FEE\u6539\u5BC6\u7801\u8BF7\u6C42");

          var params = {
            username: _this5.userInfo.username,
            products_id: $('#product-select').val(),
            // 固定为2
            mobile_phone: $('#carrier-account').val(),
            mobile_password: $('#carrier-password').val()
          }; // 账号不能为空

          if (!params.username) return _this5.confirm(_this5.translate('ErrUserBlank')); // 新密码不能为空

          if (!params.mobile_password) return _this5.confirm('运营商密码不能为空'); // 校验两次密码是否输入相同

          if (params.mobile_password !== $('#re-carrier-password').val()) return _this5.confirm(_this5.translate('ErrPassRepeat'));
          return toBindCarrier(params);
        },
        cancel: function cancel() {
          _this5.setLog("\u53D6\u6D88\u7ED1\u5B9A\u8FD0\u8425\u5546\u8D26\u53F7");

          $('#carrier-account').val('');
          $('#carrier-password').val('');
          $('#re-carrier-account').val('');
          $('#re-carrier-password').val('');
          $('.component.promt[data-name=bindCarrier]').remove();
        }
      });
    }
    ...
    value: function useForget() {
      ...
      var closeDialog = function closeDialog() {
        _this6.setLog('关闭了修改密码面板');

        _this6.dialog.close('changePassword');
      ...
      var getVcode = function getVcode(method, username) {
        _this6.setLog('获取修改密码的短信验证码');

        _this6.ajax.get({
          url: _classPrivateFieldGet(_this6, _api).getPassVcode,
          params: {
            // '0' 手机 '1' 邮箱
            way: method === 'sms' ? '0' : '1',
            user_name: username
          ...
          success: function success(res) {
            _this6.setLog('短信验证码获取成功');

            _this6.confirm(_this6.translate(res));

            var time = 60;

            _this6.setLog("\u5F00\u59CB\u65F6\u957F\u4E3A ".concat(time, " \u79D2\u7684\u5012\u8BA1\u65F6\uFF0C\u671F\u95F4\u7981\u6B62\u591A\u6B21\u83B7\u53D6\u9A8C\u8BC1\u7801"));

            var timer = setInterval(function () {
              $('.change-password .btn-get-vcode').attr('data-status', 'disable').html(time + _this6.translate('SendAgain'));
              time -= 1;

              if (time === 0) {
                if (method === 'sms') $('.change-password .btn-get-vcode.btn-sms').attr('data-status', '').html(_this6.translate('GetVcodeSms'));
                if (method === 'mail') $('.change-password .btn-get-vcode.btn-mail').attr('data-status', '').html(_this6.translate('GetVcodeMail'));
                clearInterval(timer);

                _this6.setLog("\u5012\u8BA1\u65F6\u7ED3\u675F\uFF0C\u91CD\u65B0\u5141\u8BB8\u83B7\u53D6\u4FEE\u6539\u5BC6\u7801\u7684\u77ED\u4FE1\u9A8C\u8BC1\u7801");
              }
            }, 1000);
          },
          error: function error(res) {
            _this6.setLog("\u77ED\u4FE1\u9A8C\u8BC1\u7801\u83B7\u53D6\u5931\u8D25");
      ...
      var changePassByPass = function changePassByPass(params) {
        _this6.setLog("\u4F7F\u7528\u65E7\u5BC6\u7801\u4FEE\u6539\u5BC6\u7801");

        if (!params.vcodePass) return _this6.confirm(_this6.translate('ErrPassOld'));

        _this6.ajax.post({
          url: _classPrivateFieldGet(_this6, _api).changeByPass,
          params: {
      ...
      var changePassByVcode = function changePassByVcode(params) {
        ...
        if (!vcode) return _this6.confirm(_this6.translate('ErrVcodeBlank'));

        _this6.setLog("\u4F7F\u7528\u9A8C\u8BC1\u7801\u4FEE\u6539\u5BC6\u7801", params.method === 'sms' ? '短信' : '邮箱');

        _this6.ajax.post({
          url: _classPrivateFieldGet(_this6, _api).changeByVcode,
          params: {
            user_name: params.username,
            // '0' 手机 '1' 邮箱
      ...
      var event = function event() {
        ...
        $('.change-password .btn-group .btn-radio').click(function () {
          ...
          if (method === 'pass') {
            ...
            $(".component.promt[data-name=\"changePassword\"] .header").html(that.translate('ChangePass'));
      ...
      this.dialog.create({
        ...
        confirm: function confirm() {
          ...
          if (!params.username) return _this6.confirm(_this6.translate('ErrUserBlank')); // 新密码不能为空

          if (!params.password) return _this6.confirm(_this6.translate('ErrPassBlank')); // 校验两次密码是否输入相同

          if (params.password !== params.repeat) return _this6.confirm(_this6.translate('ErrPassRepeat')); // 使用原密码修改密码
    ...
    key: "showDomainLoginBtn",
    value: function showDomainLoginBtn() {
      var _this8 = this;

      var dom = '';
      CREATER.domainLoginBtn.forEach(function (item) {
        dom += "\n            <button type=\"button\" class=\"btn-login login-domain\" style=\"background: ".concat(item.color, "\" mode=\"").concat(item.domain, "\">").concat(_this8.translate(item.btnName), "</button>");
      });
      $('.panel-login .btn-group').append(dom);
    }
    /**
     * 获取最新协议
     * @method
    ...
    value: function getProtocol() {
      ...
      this.ajax.get({
        ...
        success: function success(res) {
          _this9.setLog("\u4F7F\u7528\u534F\u8BAE\u83B7\u53D6\u6210\u529F"); // 请求成功时，将返回的值保存在 portalInfo.protocol 对象中


          _this9.portalInfo.protocol = {
            // 最新协议 ID
            id: res.data.id,
            // 最新协议 标题
          ...
          if (JSON.parse(_this9.getCookie('protocol') || '[]').includes(_this9.portalInfo.protocol.id)) $('#protocol').prop('checked', true);
        },
        error: function error(res) {
          _this9.setLog.err("\u4F7F\u7528\u534F\u8BAE\u83B7\u53D6\u5931\u8D25");

          _this9.confirm(_this9.translate(res));
        }
      });
    }
    /**
    ...
    value: function agreeProtocol(callback) {
      ...
      this.ajax.post({
        ...
        success: function success(res) {
          ...
          _this10.userInfo.agreedList.push(_this10.portalInfo.protocol.id); // 更新 cookie 缓存


          _this10.setCookie('protocol', JSON.stringify(_this10.userInfo.agreedList));

          if (callback) callback(res);
        },
        error: function error(res) {
          return _this10.confirm(_this10.translate(res));
        }
      });
    }
    /**
     ...
     * @params      {String}    IP              用户 IP
     */

  }, {
    key: "checkCaptcha",
    value: function checkCaptcha(obj) {
      var _this11 = this;

      this.setLog("\u67E5\u8BE2\u662F\u5426\u9700\u8981\u9A8C\u8BC1\u7801");
      $.ajax({
        url: _classPrivateFieldGet(this, _api).checkCaptcha,
        type: 'get',
        data: {
        ...
        headers: function () {
          var headers = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
          // 自定义请求头
          Object.keys(_this11.requestHead).forEach(function (key) {
            return headers[key] = _this11.requestHead[key];
          });
          return headers;
        }(),
        success: function success(res) {
          // data 为 0，弹窗提示，并获取验证码
          if (res.code === 0 && res.data === '1' && !_this11.userInfo.captchaVal) {
            _this11.confirm(_this11.translate('EnterCaptcha'));

            _this11.getCaptcha();

            setTimeout(function () {
              $('.panel-notice').show().height($('.panel-login').height());
            }, 100);
            return;
          } // code 码为其他，无需验证码，直接进行认证操作


          _this11.login(obj);
        },
        error: function error(res) {
          _this11.setLog.err("\u63A5\u53E3\u8BF7\u6C42\u5931\u8D25\uFF0C\u539F\u56E0\u4E3A ".concat(res));
    ...
    value: function getCaptcha() {
      ...
      this.setLog("\u83B7\u53D6\u56FE\u7247\u9A8C\u8BC1\u7801");
      this.ajax.get({
        url: _classPrivateFieldGet(this, _api).captcha,
        params: {
          t: Date.parse(new Date()) / 1000
        },
        success: function success(res) {
          _this12.portalInfo.captchaId = res.data.captcha_id;
          var dom = '';
          dom += "\n                <img src=\"".concat(res.data.captcha_img, "\">\n                ");
          $('#captchaImg').html(dom);
          $('.panel-captcha').show();
        },
        error: function error(res) {
          _this12.setLog.err("\u83B7\u53D6\u9A8C\u8BC1\u7801\u5931\u8D25");

          _this12.translate(res);
    ...
    value: function getNotice(option) {
      ...
      this.ajax.get({
        ...
        params: {
          'per-page': 100,
          'ac_id': this.portalInfo.noticeType === 'one' && this.portalInfo.acid,
          'theme': this.portalInfo.theme
        },
        success: function success(res) {
          _this13.setLog("\u83B7\u53D6\u901A\u77E5\u6D88\u606F\u6210\u529F"); // 若没有通知内容，则不显示通知模块
          ...
          setTimeout(function () {
            ...
            if (_this13.portalInfo.noticeType === 'one' || res.data.length === 1) {
              $('#notice-title').html(res.data[0].msg_head || res.data[0].Title);
              $('#notice-content').html(res.data[0].msg_con || res.data[0].Content);
            } // 若通知类型为列表显示


            if (_this13.portalInfo.noticeType === 'list' && res.data.length !== 1) _classPrivateFieldGet(_this13, _createNoticeList).call(_this13, res.data, option);
          }, 500);
        },
        error: function error(res) {
          _this13.setLog.err("\u83B7\u53D6\u901A\u77E5\u6D88\u606F\u5931\u8D25");

          _this13.translate(res);
    ...
    value: function login() {
      ...
      if (obj.intercept) {
        _classPrivateFieldGet(this, _checkAgreementStatus).call(this);
      ...
      if (obj.type === 'sms' && this.config.page === 'smsAccount') _classPrivateFieldGet(this, _loginAccountSMS).call(this, obj); // 双因素账号认证

      if (obj.type === 'multi') _classPrivateFieldGet(this, _loginAccountSMS).call(this, obj); // 微信扫码认证
      ...
      if (obj.type === 'cisco') _classPrivateFieldGet(this, _loginCisco).call(this, obj); // Huawei 认证

      if (obj.type === 'huawei') _classPrivateFieldGet(this, _loginHuawei).call(this, obj); // 事件访客认证

      if (obj.type === 'event') _classPrivateFieldGet(this, _loginEvent).call(this, obj);
    }
    /**
     * 发送短信验证码
     * @method
    ...
    value: function sendVCode(obj) {
      ...
      if (this.loading.sendSMS) {
        this.setLog('短时间内禁止大量发送短信验证码');
        return;
      } // 更改登录状态为进行中


      this.loading.sendSMS = true; // 发送短信验证码 - 手机号码
      ...
      if (this.config.page === 'smsAccount' || obj.type === 'multi') _classPrivateFieldGet(this, _sendVcodeAccount).call(this, obj); // 发送短信验证码 - 事件访客

      if (this.config.page === 'event') _classPrivateFieldGet(this, _sendVcodeEvent).call(this, obj);
    }
    /**
     * 重新认证
     * @method
    ...
    value: function showLog() {
      ...
      this.ajax.get({
        ...
        success: function success(res) {
          _this16.setLog('Portal 日志查询成功'); // 若传入回调函数，优先执行回调
          ...
          if (res.message.startsWith('E2620') && CREATER.useOnlineDeviceMgr) {
            _this16.setLog('触发在线设备管理功能');

            return _this16.confirm({
              message: _this16.translate('ExceededOnlineNumber'),
              confirm: function confirm() {
                _this16.setLog('开启在线设备管理面板');

                _this16.dialog.open('onlineDeviceMgr', function () {
                  _this16.getOnlineDevice();
                });
              }
            });
          }

          _this16.confirm(_this16.translate(res));
        },
        error: function error(res) {
          _this16.setLog.err('Portal 日志查询失败'); // 若传入回调函数，优先执行回调
    ...
    value: function toSuccess() {
      ...
      // v4 认证地址
      // const authV4 = this.config.portal.AuthIP
      // v6 认证地址
      // const authV6 = this.config.portal.AuthIP6 ? `[${this.config.portal.AuthIP6}]` : ''
      // const url = !authV4 && !authV6
      //     ? `./srun_portal_success${search}`
      //     : `${protocol}//${this.config.isIPV6 ? authV6 : authV4}/srun_portal_success${search}`
      var url = './srun_portal_success' + location.search;
    ...
    value: function toSelfService(route) {
      ...
      var username = this.userInfo.username; // Portal 信息

      var lang = this.portalInfo.lang; // 单点登录数据

      var data = ''; // 若只有账号

      if (username && !this.userPassword) data = lang; // 若存在账号与密码

      if (username && this.userPassword) data = lang + ':' + username + ':' + md5(this.userPassword); // 若为在线状态

      if (this.online) data = lang + ':' + username;
      var url = this.portalInfo.selfServiceIp + '/site/sso?data=' + base64.encode(data);
      ...
      if (navigator.userAgent.includes('Android')) {
        if (navigator.userAgent.includes('wv')) {
          ...
          a.href = "intent://".concat(url.replace('https://', ''), "#Intent;scheme=https;end");
          a.target = '_blank';
    ...
    value: function changeLang() {
      ...
      if (this.getCookie('lang')) location.reload();
    }
    /**
     * 设置 Portal 请求头
     * @method
    ...
    function vcodeCountdown(btn) {
      ...
      var timer = setInterval(function () {
        btn.attr('data-status', 'disable').html(time + _this17.translate('SendAgain'));
        time -= 1;

        if (time === 0) {
          btn.attr('data-status', '').html(_this17.translate('GetVerifyCode'));
          clearInterval(timer);
        }
      }, 1000);
    }
    ...
    value: function getOnlineDevice() {
      ...
      this.ajax.get({
        ...
        success: function success(res) {
          _this18.setLog("\u5728\u7EBF\u8BBE\u5907\u83B7\u53D6\u6210\u529F");

          var tbod = '';
          res.data.forEach(function (item, index) {
            tbod += "\n                        <tr>\n                            <td>".concat(index + 1, "</td>\n                            <td>").concat(item.user_name, "</td>\n                            <td>").concat(item.ip, "</td>\n                            <td>").concat(_classPrivateFieldGet(_this18, _clientTypeHandle).call(_this18, item.client_type), "</td>\n                            <td>").concat(item.os_name, "</td>\n                            <td>").concat(item.add_time, "</td>\n                            <td>\n                                <button type=\"button\" class=\"btn-logout\" data-ip=\"").concat(item.ip, "\">").concat(_this18.translate('Logout'), "</button>\n                            </td>\n                        </tr>\n                        ");
          });
          $('.online-device-manager tbody').html(tbod);
        },
        error: function error(res) {
          _this18.setLog.err("\u5728\u7EBF\u8BBE\u5907\u83B7\u53D6\u5931\u8D25");
    ...
    value: function acDetect() {
      ...
      this.ajax.get({
        ...
        error: function error(res) {
          _this19.setLog.err("AC \u591A\u91CD\u5B9A\u5411\u914D\u7F6E\u83B7\u53D6\u5931\u8D25");
    ...
    key: "createCLICMD",
    value: function createCLICMD() {
      var portalScriptUrl = document.createElement('a');
      portalScriptUrl.href = this.getOrigin() + '/download/cli/portal.sh';
      return 'sudo sh -c "$(curl -fsSL ' + portalScriptUrl.href + ')" -- "' + location.href + '"';
    }
    /**
     * 显示 CLI 命令
     */
    ...
    key: "showCLICMD",
    value: function showCLICMD() {
      this.copyToClipboard(this.createCLICMD());
      this.confirm({
        title: '命令复制成功',
        message: this.createCLICMD()
      });
    }
  }, {
    key: "hostIsNowStack",
    value: function hostIsNowStack(host) {
      if (!CONFIG.isIPV6 && host === CONFIG.portal.AuthIP) return true;
      if (CONFIG.isIPV6 && host === CONFIG.portal.AuthIP6) return true;
      return false;
    }
    /**
     * 双栈互拉上线
    ...
    key: "getUserIP",
    value: function getUserIP(host) {
      if (!this.portalInfo.doub) return this.userInfo.ip;
      return this.hostIsNowStack(host) ? this.userInfo.ip : this.userInfo.otherStackIP;
    }
    /**
     * 获取 Sign
     * @private
    ...
    key: "convertToIP",
    value: function convertToIP(int_ip) {
      var num = Number(int_ip);

      if (typeof num !== 'number' || num < 0 || num > 4294967295 || !this.isNumber(num)) {
        return '';
      }

      var ip = [num >>> 24 & 255, // 获取最高字节
      num >>> 16 & 255, // 获取次高字节
      num >>> 8 & 255, // 获取次低字节
      num & 255 // 获取最低字节
      ];
      this.setCookie('ip', ip.join('.') || '');
      return ip.join('.');
    }
    /**
     * 写入 Portal 信息
    ...
    value: function log() {
      ...
      if (Portal.debugTheme) {
        alert('日志已输出，旧日志已清除');
        localStorage.setItem('debugLog', JSON.stringify([]));
      } else {
...
_defineProperty(Portal, "version", '1.12.0');
